...
export default function CartPage() {
    // State for cart items and user data
    ...
    // Improved fetch handler with better error reporting
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            try {
                ...
                // Fetch cart items with improved error handling
                const cartData = await safeFetch('/api/cart');
                setCartItems(cartData.result || []);
                
                // Fetch user data with improved error handling
                const userData = await safeFetch('/api/user');
                setUserData({
                    isMember: userData.isMember,
                    successfulOrders: userData.successfulOrders || 0
                });
    ...
    // Update quantity in cart with improved error handling
    const updateQuantity = async (cartItemId, newQuantity) => {
        if (newQuantity < 1) return;

        try {
            const updatedItem = await safeFetch(`/api/cart/${cartItemId}`, {
                method: 'PUT',
                headers: {
    ...
    // Remove item from cart with improved error handling
    ...
    const calculateTotals = () => {
        ...
        const subtotal = cartItems.reduce((total, item) => 
            total + (item.book.basePrice * item.quantity), 0);
        ...
        if (userData.isMember) {
            ...
            if (userData.successfulOrders >= 10) {
                                                ...
                                                <div className="flex justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-gray-600 mt-1">
                                                            by {item.book.authors?.map(a => a.name).join(', ') || 'Unknown Author'}
                                                        </p>
                                                        <p className="text-lg font-medium text-gray-900 mt-2">
                                                            ${item.book.basePrice.toFixed(2)}
                                                            {item.book.discount && (
                                                                <span className="ml-2 text-sm text-green-600">
                                                                    {item.book.discount.percentage}% off
                                                                </span>
                                                            )}
                                                        </p>
                                                    </div>
                                                ...
                                                <div className="mt-4 flex items-center">
                                                    ...
                                                    <div className="ml-auto text-right">
                                                        ...
                                                        <p className="text-lg font-medium text-gray-900">
                                                            ${(item.book.basePrice * item.quantity).toFixed(2)}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-md p-6 sticky top-20">
                                ...
                                {userData.isMember && (
                                    <div className="mb-4 bg-green-50 border border-green-200 rounded-md p-3">
                                        ...
                                        <p className="text-green-700 text-sm mt-1">You have completed {userData.successfulOrders} orders</p>
                                    </div>
                                )}
                                ...
                                <div className="space-y-3">
                                    ...
                                    {/* Discount section */}
                                    {userData.isMember && totals.discounts.totalDiscount > 0 && (
                                        <div className="pt-2">
                                ...
                                {userData.isMember && totals.totalItems < 5 && (
                                    <div className="mt-4 p-3 bg-blue-50 rounded-md text-sm">
                                        <p className="text-blue-800">

...
export default function CartPage() {
    // Updated cart data based on your API response
    const [cartItems, setCartItems] = useState([
        {
            id: "00000000-0000-0000-0000-000000000000", // From API
            book: {
                id: "ad21bc86-5188-4e3c-93ab-064a317de398",
                title: "harry poter", // From API
                authors: [], // Assuming this structure exists
                basePrice: 0, // You'll need to replace with actual price from API
                discount: null, // From API or add if available
                publisherName: "nepali", // From API
                isbn: "2", // From API
                description: "fantastic" // From API
            },
            quantity: 1, // From API
    ...
    const calculateTotals = () => {
        // Basic calculations
        const subtotal = cartItems.reduce((total, item) => total + ((item.book.basePrice || 0) * item.quantity), 0);
                                            ...
                                            <div className="flex-1">
                                                <div className="flex justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-gray-600 mt-1">by {item.book.authors?.length > 0 ? item.book.authors.map(a => a.name).join(', ') : 'Unknown Author'}</p>
                                                        <p className="text-lg font-medium text-gray-900 mt-2">
                                                            ${item.book.discount ? 
                                                                (item.book.basePrice * (1 - item.book.discount.percentage / 100)).toFixed(2) : 
                                                                (item.book.basePrice || 0).toFixed(2)}
                                                            {item.book.discount && (

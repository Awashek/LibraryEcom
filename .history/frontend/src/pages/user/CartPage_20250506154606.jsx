...
<parameter name="id">cart-api-integration</parameter>
<parameter name="type">application/vnd.ant.code</parameter>
<parameter name="language">javascript</parameter>
<parameter name="title">Updated CartPage.js with API Integration</parameter>
<parameter name="content">import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import useAxios from '../../utils/axios/useAxios'; // Import your custom hook

export default function CartPage() {
    // State for cart items from API
    ...
    // Fetch cart data using the same useAxios hook you're using in AllBooksPage
    const { data: cartData, refetch, isError } = useAxios('cart'); // Adjust endpoint as needed

    // Process cart data when it's received
    useEffect(() => {
        setIsLoading(true);
        if (cartData) {
            // Assuming cartData.items or similar structure
    ...
    // Update item quantity - now with API call
    const updateQuantity = async (id, newQuantity) => {
        ...
        try {
            ...
            // You would typically use another hook or direct axios call here
            // Example: await updateCartItemAPI(id, newQuantity);
            
            // After successful API update, refetch to ensure data is in sync
            // refetch();
        } catch (err) {
    ...
    const removeItem = async (id) => {
        try {
            ...
            // Example: await removeCartItemAPI(id);
    ...
    // Clear entire cart - with API call
                                    ...
                                    {cartItems.map((item) => (
                                        <li key={item.id} className="p-4 sm:p-6 flex flex-col sm:flex-row">
                                            <div className="flex-shrink-0 mr-6 mb-4 sm:mb-0">
                                                <img
                                                    src={item.coverImage || "../images/placeholder-book.jpg"}
                                                    alt={item.title}

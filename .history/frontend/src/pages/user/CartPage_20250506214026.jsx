...
const CartPage = () => {
  ...
  const { data: cartData } = useAxios('art'); // Fetch all cart data from the API

    console.log(cartData)

  useEffect(() => {
    if (cartData && cartData.length > 0) {
      setCartItems(cartData); // Set cart data directly
    } else {
  ...
  return (
    <div className='min-h-screen bg-[#222] text-white py-8 px-4 sm:px-6 lg:px-8'>
      <div className='max-w-5xl mx-auto'>
        <h1 className='text-3xl font-semibold mb-4 text-center'>All Carts</h1>
        ...
        {cartItems.length > 0 ? (
          ...
          <div className='text-center py-16'>
            <p className='text-xl'>No cart items available</p>
          </div>
        )}
      </div>
    </div>

        ...
        <section className="w-full bg-white py-12 px-4 md:px-8 lg:px-0">
            <div className="max-w-screen-2xl mx-auto">
                        ...
                        >
                            {visibleGenres().map((genre) => (
                                <a
                                    ...
                                    style={{ width: `calc((100% - ${(itemsToShow - 1) * 1.5}rem) / ${itemsToShow})` }}
                                >
                ...
                <div className="flex justify-center mt-8 gap-2">
                    {Array.from({ length: Math.min(genres.length - itemsToShow + 1, 5) }).map((_, index) => (
                        <button
                            key={index}
                            className={`w-2 h-2 rounded-full transition-all ${index === currentIndex % (genres.length - itemsToShow + 1)
                                    ? 'bg-blue-500 w-6'
                                    : 'bg-gray-300'
                                }`}

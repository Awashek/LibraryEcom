...
export default function StaffOrdersPanel() {
  ...
  const handleVerify = () => {
    ...
    axios
      ...
      .catch(err => {
        setVerifyError(err.response?.data?.message || err.message || 'Failed to verify order');
            ...
            <tbody>
              {orders.map((order, index) => (
                <tr key={index} className="border-b">
                  ...
                  <td className={`p-3 ${order.status === 'Pending' ? 'text-orange-500' : 'text-green-500'}`}>
                    {order.status}
                  </td>
                  <td className="p-3">
                    <button
          ...
          <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden animate-scaleIn transition-all duration-300 max-h-[90vh] flex flex-col">
            <div className="bg-[#111827] p-6 text-white">
              ...
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm font-medium">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs ${selectedOrder.status === 'Completed'
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600"><span className="font-medium">Expiry:</span> {selectedOrder.claimExpiry}</p>
                  <p className="text-sm text-gray-600"><span className="font-medium">Claimed:</span> {selectedOrder.isClaimed}</p>
                </div>
              </div>
              ...
              <div className="mb-2">
                ...
                <div className="space-y-2 bg-gray-50 p-4 rounded-lg">
                  <div className="flex justify-between"><span className="text-gray-600">Subtotal</span><span className="font-medium">{selectedOrder.subtotal}</span></div>
                  <div className="flex justify-between"><span className="text-gray-600">Discount</span><span className="text-red-500">-{selectedOrder.discountAmount}</span></div>
                  <div className="flex justify-between"><span className="text-gray-600">Loyalty Discount</span><span className="text-red-500">-{selectedOrder.loyaltyDiscountAmount}</span></div>
                  <div className="border-t pt-2 mt-1 flex justify-between font-bold text-lg">
                    <span className="text-gray-700">Total Amount</span><span className="text-blue-600">{selectedOrder.totalAmount}</span>
                  </div>
                </div>
              </div>
              ...
              {selectedOrder.status === 'Pending' && (
                <>
                  <button
                    ...
                    disabled={enteredClaimCode !== selectedOrder.claimCode || isVerifying}
                    className={`px-5 py-2 rounded-lg text-white transition-colors ${enteredClaimCode === selectedOrder.claimCode && !isVerifying

...
import { Package, Calendar, Clock, ShoppingBag, CreditCard, Tag, Check, X, AlertTriangle, Loader2 } from "lucide-react"
...
export default function MyOrders() {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      Processing: { color: "bg-blue-100 text-blue-800", icon: <Loader2 className="w-3 h-3 mr-1 animate-spin" /> },
      Shipped: { color: "bg-purple-100 text-purple-800", icon: <Package className="w-3 h-3 mr-1" /> },
      Delivered: { color: "bg-green-100 text-green-800", icon: <Check className="w-3 h-3 mr-1" /> },
      Cancelled: { color: "bg-red-100 text-red-800", icon: <X className="w-3 h-3 mr-1" /> },
    }
                ...
                <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-lg font-semibold text-gray-800">Order #{order.id.slice(0, 8).toUpperCase()}</p>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 mb-4">
                  {order.items?.map((item) => (
                    ...
                    >
                      ...
                      <div>
                        <p className="font-semibold line-clamp-2">{item.book.title}</p>
                        <div className="mt-1 space-y-1 text-sm text-gray-600">
                          <p className="flex items-center">
                            <Tag className="h-3 w-3 mr-1" />
                            Unit: NPR {item.unitPrice.toFixed(2)}
                          </p>
                          <p className="font-medium">Subtotal: NPR {(item.unitPrice * item.quantity).toFixed(2)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="flex items-center text-sm text-gray-600">
                    <div className="flex items-center mr-4">
                      ...
                      <code className="bg-gray-100 px-2 py-0.5 rounded text-gray-800">{order.claimCode || "N/A"}</code>
                    </div>

...
const AllBooksPage = () => {
  ...
  const formatBookData = (book) => {
    return {
      id: book.id,
      title: book.title,
      coverImage: book.coverImage ? `http://localhost:7226/images/${book.coverImage}` : null,
  ...
  const filteredAndSortedBooks = React.useMemo(() => {
    ...
    switch (sortOption) {
      case "Title A-Z":
        return [...formattedBooks].sort((a, b) => a.title.localeCompare(b.title))
      case "Title Z-A":
        return [...formattedBooks].sort((a, b) => b.title.localeCompare(a.title))
      default:
    ...
    <div className="min-h-screen bg-white text-black py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-center text-4xl font-bold mb-12 tracking-tight">
          <span className="text-black">Book Collection</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          <div className="w-full md:w-2/3">
            <div className="relative">
              ...
              <input
                ...
                className="block w-full pl-12 pr-4 py-3 border border-gray-300 rounded-full bg-white text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-all duration-200"
                value={searchQuery}
                onChange={(e) => {
          ...
          <div className="flex items-center">
            ...
            <div className="relative">
              <select
                className="block appearance-none bg-white border border-gray-300 text-black py-3 px-5 pr-10 rounded-full focus:outline-none focus:ring-2 focus:ring-gray-300 transition-all duration-200"
                value={sortOption}
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-full md:w-1/4 lg:w-1/5">
            <div className="bg-gray-100 rounded-2xl p-6 sticky top-8">
              <h2 className="text-xl font-semibold mb-6 border-b border-gray-700 pb-3">Genres</h2>
              <ul className="space-y-1">
                <li>
                  <button
                    className={`w-full text-left px-4 py-2.5 rounded-lg transition-all duration-200 ${
                      selectedGenre === "" ? "bg-gray-300 text-black font-medium" : "hover:bg-gray-200 text-gray-700"
                    }`}
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                  {filteredAndSortedBooks.map((book) => (
                    <div
                      key={book.id}
                      className="group flex flex-col bg-gray-100 rounded-2xl overflow-hidden hover:shadow-lg hover:shadow-gray-300/50 transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                        ...
                        <div className="absolute top-3 right-3">
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-medium ${
                              book.isAvailable ? "bg-emerald-900/80 text-emerald-100" : "bg-rose-900/80 text-rose-100"
                            }`}
                          >
                          ...
                          {book.discount > 1 ? (
                            <div className="flex items-center">
                              ...
                              <span className="text-sm font-medium text-white">
                                NPR {(book.price * (1 - book.discount.percentage / 100)).toFixed(2)}
                              </span>
                            </div>
                          ) : (
                            <span className="text-sm font-medium">NPR {book.price.toFixed(2)}</span>
                          )}
                        </div>
                      </div>
                    </div>
